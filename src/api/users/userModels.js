		...
		bcrypt
			...
			.then(function (hashedPassword) {
				let wialonHost = userDetails?.wialon_host
					...
					: "";
				var sql = `BEGIN;
                insert into users (username, password, name, email,wialon_host,department_id,phone,ip_phone,birthdate,created_at,account_name,wialon_username,telegram_username,default_route,allow_send_email,is_group_base_role,telegram_id) 
                values ("${userDetails?.username}", "${hashedPassword}", "${
					userDetails?.name
				}","${userDetails?.email}" ,"${
					userDetails?.wialonHost ? userDetails?.wialonHost : ""
				}" 
                ,${userDetails?.depId},"${userDetails?.phone}","${
					userDetails?.ip_phone
				}","${userDetails?.birthdate}", "${getDateTime()}","${
					userDetails?.account_name
						? userDetails?.account_name
						: ""
				}","${
					userDetails?.wialon_username
						? userDetails?.wialon_username
						: ""
				}","${
					userDetails?.telegram_username
						? userDetails?.telegram_username
						: ""
				}","${
...
function updateUserDetails(data, req, callback) {
	...
	var sql = `BEGIN;
        update users set name= ${name},email=${email}, department_id=${dep_id},phone=${phone},ip_phone=${ip_phone}
        ,birthdate=${birthdate}, telegram_username=${telegram_username}, default_route=${default_route}, updated_at="${getDateTime()}" , allow_send_email=${allow_send_emails},is_group_base_role=${is_group_base_role},telegram_id=${telegram_id} where user_id=${
		req.params.user_id
	};
        INSERT INTO activities (user_id,user_id_affected,log_type_id,note,created_at) 
            VALUES(${data.user_id}, ${
...
function getUsersList(dataFields, callback) {
	var haveRole = dataFields?.haveRole,
		department_id = dataFields?.department_id
			? dataFields?.department_id
			: 0,
		...
		has_telegram_id = dataFields?.has_telegram_id
			...
			: false;
	var sql = `select a.name,a.phone,a.email,a.user_id,a.department_id,a.is_group_base_role,a.telegram_id from users a  where a.deleted_at is null `;
	if (department_id) {
	...
	getUserRoles(from_user_id, (getRolesResult) => {
		...
		if (getRolesResult && getRolesResult.length)
			removeOldPermissionsForUsers(to_user_ids, (result) => {
				...
				getRoles((roles) => {
					// //console.log(roles)
					const to_user_ids_array = to_user_ids
						.split(",")
						.map(Number);
