    ...
    bcrypt
      ...
      .then(function (hashedPassword) {
        ...
        var sql = `BEGIN;
          ...
          userDetails?.wialon_username ? userDetails?.wialon_username : ""
        }","${
          userDetails?.telegram_username ? userDetails?.telegram_username : ""
          ...
          userDetails?.telegram_id ? userDetails?.telegram_id : null
        });
...
function updateUserDetails(data, req, callback) {
  var name = req.body.name ? '"' + req.body.name + '"' : "name",
    ...
    ip_phone = req.body.ip_phone ? '"' + req.body.ip_phone + '"' : "ip_phone",
...
function getUsersList(dataFields, callback) {
  var haveRole = dataFields?.haveRole,
    ...
    has_telegram_id = dataFields?.has_telegram_id
      ...
      : false;
  var sql = `select a.name,a.user_id,a.department_id,a.is_group_base_role,a.telegram_id from users a  where a.deleted_at is null `;
  if (department_id) {
