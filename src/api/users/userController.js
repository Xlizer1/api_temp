    ...
    if (data) {
      ...
      try {
        var is_internal_or_external = searchFields?.is_internal_or_external
          ? searchFields?.is_internal_or_external
          : 0;
      } catch (error) {
...
function getUsersList(req, response) {
  auth.verify(req.headers["jwt"], (data) => {
    if (data) {
      ...
      try {
        var department_id =
          req?.query && req?.query?.department_id
      ...
      try {
        var has_telegram_id =
          req?.query && req?.query?.has_telegram_id
            ? req?.query?.has_telegram_id
            : 0;
      } catch (err) {
      ...
      try {
        var is_engineer =
          req && req?.query && req?.query?.is_engineer
            ? req?.query?.is_engineer
            : 0;
      } catch (err) {
        var is_engineer = 0;
      }
      try {
        var enabled =
          req && req?.query && req?.query?.enabled ? req?.query?.enabled : 0;
      } catch (err) {
        var enabled = 0;
      }
          ...
          if (data.roles_id.includes(2)) {
            mod.updateUserPassword(data, user_id, new_password, (result) => {
              if (result)
                response({
                  status: true,
                  data: { message: msg.passwordUpdated },
                });
              else response({ status: false, data: { message: msg.error } });
            });
            ...
            mod.checkOldPassword(user_id, old_password, (result) => {
              if (result) {
                ...
                response({
                  status: false,
                  data: { message: msg.invalidPassword },
                });
            });
          } else
            response({ status: false, data: { message: msg.unauthorized } });
  ...
  auth.verify(req.headers["jwt"], (data) => {
    if (data) {
      if (data.roles_id.includes(1) || data.roles_id.includes(2)) {
        try {
          var from_user_id =
            req && req.body && req.body.from_user_id
        ...
        try {
          var to_user_ids =
            req && req.body && req.body.to_user_ids ? req.body.to_user_ids : [];
        } catch (err) {
          var to_user_ids = [];
        }
        try {
          var copy_as_group_base =
            req && req.body && req.body.copy_as_group_base
              ? req.body.copy_as_group_base
              : 0;
        } catch (err) {
        ...
        if (!to_user_ids) {
          if (messageError) messageError += ", and To users";
          else messageError = "You must select To users";
        }
        if (messageError) response(getRes(false, null, messageError));
        if (copy_as_group_base == 0) {
          mod.copyPermissionsForAnotherUsers(
            ...
            (result) => {
              if (
                result &&
                result != "no roles" &&
                result != "not all permissions"
              )
                return response(getRes(true, null, msg.updated));
              else if (result == "no roles") {
                return response(
                  getRes(false, null, msg.noRolesForSelectedUser)
                );
          ...
          mod.copyPermissionsForAnotherUsersByGroup(
            ...
            (result) => {
              if (
                result &&
                result != "no groups or user not base on groups" &&
                result != "not all permissions"
              )
                return response(getRes(true, null, msg.updated));
  ...
  auth.verify(req.headers["jwt"], (data) => {
    if (data) {
      if (
        ...
        try {
          var allow_send_email =
            req && req.body && req.body.allow_send_email
              ? req.body.allow_send_email
              : 0;
        } catch (err) {
